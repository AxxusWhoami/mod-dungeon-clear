...
std::unordered_map<uint64, std::vector<DungeonBossInfo>> const& BossSpawnIndex::Store()
{
    static auto const kStore = []{
        std::unordered_map<uint64, std::vector<DungeonBossInfo>> store;
        ...
        for (auto const& kv : spawns)
        {
            ...
            for (auto it = range.first; it != range.second; ++it)
            {
                ...
                info.y = data.posY;
                info.z = data.posZ;
                store[MakeKey(row.mapId, row.difficulty)].push_back(info);
            }
        }
...
std::vector<DungeonBossInfo> const& BossSpawnIndex::Get(uint32 mapId, Difficulty difficulty)
{
    ...
    auto it = store.find(MakeKey(mapId, difficulty));
    if (it == store.end())
        return empty;
    return it->second;
}
